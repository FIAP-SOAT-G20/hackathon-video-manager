...
export const POST = async function createUser(request) {
  try {
    ...
    // Use constant user_id instead of extracting from token
    const user_id = 2;

    // Create user in user service
    const userData = await makeUserServiceRequest('/api/v1/user', {
      method: 'POST',
      body: JSON.stringify({
        name,
        email,
...
export const GET = async function getUser(request) {
  try {
    const session = await auth0.getSession();
    ...
    const userData = await makeUserServiceRequest(`/api/v1/user/${userId}`);
